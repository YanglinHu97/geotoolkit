...
def test_spatial_indexing():
    ...
    # Note: nearest() takes (geometry, geometry), so we extract the polygon geometry
    ...
    # Note: nearest_optimized() takes (geometry, FeatureCollection)
...
def test_geometric_features():
    ...
    # (Note: This might not hold for U-shaped polygons, but holds for the sample square)
    assert shape(poly).contains(shape(cent))
    
    # 2. Test Envelope
    env = get_envelope(poly)
    assert env["type"] == "Polygon"
    # Envelope area must be >= Polygon area
